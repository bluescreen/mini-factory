...
export function phase(role, model, text) {
  ...
  save(`${n}-${role}.json`, JSON.stringify({ phase: role, model, usd: total_cost_usd, ms: duration_ms, usage, answer }, null, 2));
...
function ask(model, text) {
  return new Promise((resolve, reject) => {
    const child = spawn('claude', ['-p', text, '--tools', '', '--output-format', 'json', '--model', model], { stdio: ['ignore', 'pipe', 'pipe'] });
    let out = '';
    let err = '';
...
export async function panel(role, model, prompts) {
  ...
  return answers.map((stdout, i) => {
    ...
    const { result, total_cost_usd = 0, duration_ms = 0, usage = {} } = JSON.parse(stdout);
    ledger.push({ n, role, model, usd: total_cost_usd, ms: duration_ms, out: usage.output_tokens ?? 0, par: true });
    save(`${n}-${role}.json`, JSON.stringify({ phase: role, model, usd: total_cost_usd, ms: duration_ms, par: true, usage, answer: result.trim() }, null, 2));
    console.log(`  ${n}  AGENT  ${role.padEnd(6)}  ${model}`);
    return result.trim();
  });
}
...
export function gate(command = 'npm test') {
  ...
  const pass = status === 0;
  save(`${n}-test.json`, JSON.stringify({ phase: 'test', command, pass, ms: Date.now() - started, output: output.slice(-2000) }, null, 2));
...
export function commit(goal, plan, review = '', branch = `factory/${RUN}`) {
  const n = next();
  const started = Date.now();
  const git = (...args) => spawnSync('git', args, { encoding: 'utf8' });
  writeFileSync('PLAN.md', `# ${goal}\n\n${plan.trim()}\n`);
  git('checkout', '-b', branch);
  git('add', '-A');
  const subject = `feat: ${goal.trim().split('\n')[0]}`;
  ...
  if (process.env.FACTORY_PR) {
    ...
    if (pushed.status !== 0) {
      ...
      const made = spawnSync('gh', ['pr', 'create', '--repo', slug, '--title', subject, '--body', body, '--head', branch], { encoding: 'utf8' });
